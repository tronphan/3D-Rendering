...
def calc_transform(s_pose, t_pose):
    if s_track_confidence == 3 and t_track_confidence == 3:
        ...
        """
        trans = np.array([  [-(1 - 2 * q.y*q.y - 2 * q.z*q.z),    2 * q.x*q.y - 2 * q.z*q.w,      -(2 * q.x*q.z + 2 * q.y*q.w),       t.x],
                            [-(2 * q.x*q.y + 2 * q.z*q.w),        1 - 2 * q.x*q.x - 2 * q.z*q.z,  -(2 * q.y*q.z - 2 * q.x*q.w),       t.y],
                            [-(2 * q.x*q.z - 2 * q.y*q.w),        2 * q.y*q.z + 2 * q.x*q.w,      -(1 - 2 * q.x*q.x - 2 * q.y*q.y),   t.z],
                            [0,                                   0,                              0,                                  1  ]  ])        
        info = np.identity(4)
